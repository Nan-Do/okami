...
DELIMITER = '%%'
SOURCE_DIRECTORY = "Java_Template"
#OUTPUT_DIRECTORY = "./"
...
def emitCodeDataStructureGetNodeWithReturnValue(outfile, length, spaces_level, return_value):
    for x in xrange(1, length):
        ...
        outfile.write('{}Node{} {} = {}.get(x_{});\n'.format(SPACES * spaces_level,
                                                           x + 1,
                                                           node_str,
                                                           level_str,
                                                           x))
        outfile.write('{}if ({} == null) return {};\n'.format(SPACES * spaces_level, 
                                                              node_str, 
                                                              return_value))
...
def fillIntList(outfile, spaces):
    ...
    if length > 0:
        ...
        outfile.write('{}ArrayLockedListInt {};\n'.format(spaces, l_args))
...
def fillRewritingVariable(outfile):
    ...
    outfile.write('{}public VariableType v_type;\n'.format(SPACES))
    #outfile.write('{}public int [] values = new int[{}];\n\n'.format(SPACES, max_length))
    outfile.write('{}public int [] values;\n\n'.format(SPACES))
    
    outfile.write('{}public RewritingVariable(){{\n'.format(SPACES))
    outfile.write('{}this.v_type = null;\n'.format(SPACES * 2))
    ...
    for level_length in xrange(1, max_length + 1):
        ...
        outfile.write('{}public RewritingVariable(VariableType v_type, {}){{\n'.format(SPACES, n_vars))
...
def fillViews(outfile):
    ...
    outfile.write('public class Views {\n')
    for x, view_name in enumerate(view_names):
        outfile.write('{} final public static int {} = {};\n'.format(SPACES, view_name, x))
    outfile.write('}\n')
...
def fillVariableType(outfile):
    ...
    predicate_ids_str = ', '.join('{}({})'.format(name, pos) for pos, name in enumerate(map(lambda x: x.name.upper(),
                                                                                            predicate_ids)))
    outfile.write('public enum VariableType {\n')
    outfile.write('{}{};\n\n'.format(SPACES, predicate_ids_str))
    
    outfile.write('{}private VariableType(int id){{\n'.format(SPACES))
    outfile.write('{}this.ID = id;\n'.format(SPACES * 2))
    outfile.write('{}}}\n\n'.format(SPACES))
    
    outfile.write('{}public int ID;\n}}'.format(SPACES))

def fillDataStructureCreateNodeClasses(outfile):
    ...
    for length in lengths:
        ...
        outfile.write('{}private class Node{} {{\n'.format(SPACES, length))
        ...
        if length != number_of_data_structure_nodes:
            outfile.write('{0}public ConcurrentHashMap<Integer, Node{1}> level{1};\n'.format(SPACES * 2, length + 1))
            outfile.write('{0}public Lock hashmap_lock;\n'.format(SPACES * 2))
            outfile.write('\n')
            
        for view_name in view_names:
            outfile.write('{}public ArrayLockedListInt {};\n'.format(SPACES * 2, view_name))
          
        outfile.write('\n')
          
        for solution_name in solution_names:
            #outfile.write('{}public LockBitSet solution_{};\n'.format(SPACES * 2, solution_name))
            outfile.write('{}public LockBitSet solution_{};\n'.format(SPACES * 2, solution_name))
        
        outfile.write('\n')
        
        outfile.write('{}Node{}() {{\n'.format(SPACES * 2, length))
        if length != number_of_data_structure_nodes:
            outfile.write('{0}level{1} = new ConcurrentHashMap<Integer, Node{1}>();\n'.format(SPACES * 3, length + 1))
            outfile.write('{}hashmap_lock = new ReentrantLock();\n'.format(SPACES * 3))
            outfile.write('\n')
            
        for view_name in view_names:
            outfile.write('{}{} = new ArrayLockedListInt();\n'.format(SPACES * 3, view_name))
        outfile.write('\n')
        
        for solution_name in solution_names:
            #outfile.write('{}public solution_{} = new LockBitSet();\n'.format(SPACES * 3, solution_name))
            outfile.write('{}solution_{} = new LockBitSet();\n'.format(SPACES * 3, solution_name))
        
        outfile.write('{}}}\n'.format(SPACES * 2))
        outfile.write('{}}}\n\n'.format(SPACES))
        
def fillDataStructureConstructor(outfile):
    ...
    if root_hashtable:
        outfile.write('{}ArrayLockedListInt empty_list;\n'.format(SPACES))
        outfile.write('{}ConcurrentHashMap<Integer, Node2> root;\n'.format(SPACES))
        outfile.write('{}Lock hashmap_lock;\n'.format(SPACES))
    
    for pos, variable_id in enumerate(answers_of_length_1.union(predicates_in_rules_of_length_1)):
        if pos == 0 and root_hashtable:
            outfile.write('\n')
                        
        outfile.write('{}Set<Integer> R_{};\n'.format(SPACES,
                                                      variable_id.name))
        outfile.write('{}Lock R_{}_lock;\n'.format(SPACES,
                                                   variable_id.name))
    outfile.write('\n')
    
    outfile.write('{}Datastructure() {{\n'.format(SPACES))
    if root_hashtable:
        outfile.write('{}empty_list = new ArrayLockedListInt();\n'.format(SPACES * 2))
        outfile.write('{}root = new ConcurrentHashMap<Integer, Node2>();\n'.format(SPACES * 2))
        outfile.write('{}hashmap_lock = new ReentrantLock();\n'.format(SPACES * 2))
    ...
    for pos, variable_id in enumerate(answers_of_length_1.union(predicates_in_rules_of_length_1)):
        ...
        outfile.write('{}R_{} = ConcurrentHashMap.newKeySet();\n'.format(SPACES * 2,
                                                                         variable_id.name))
        outfile.write('{}R_{}_lock = new ReentrantLock();\n'.format(SPACES * 2,
                                                                    variable_id.name))
...
def fillDataStructureInsertFunctions(outfile):
    ...
    for length in lengths:
        ...
        if length == 1:
            outfile.write('{}void insert_1(int x_1) {{\n'.format(SPACES))
            
            if root_hashtable:
                outfile.write('{}Node2 node = root.get(x_1);\n'.format(SPACES * 2))
                outfile.write('{}if (node == null) {{\n'.format(SPACES * 2))
                outfile.write('{}hashmap_lock.lock();\n'.format(SPACES * 3))
                outfile.write('{}node = root.get(x_1);\n'.format(SPACES * 3))
                outfile.write('{}if (node == null) {{\n'.format(SPACES * 3))
                outfile.write('{}node = new Node2();\n'.format(SPACES * 4))
                outfile.write('{}root.put(x_1, node);\n'.format(SPACES * 4))
                outfile.write('{}}}\n'.format(SPACES * 3))
        ...
        emitCodeDataStructureGetNodeCreateIfDoesntExist(outfile, length, 2)
...
def fillDataStructureGetLevel0Values(outfile):
    ...
    outfile.write('{}List<Integer> get_level0_values() {{\n'.format(SPACES))
    outfile.write('{}return Collections.list(root.keys());\n'.format(SPACES * 2))
    outfile.write('{}}}\n'.format(SPACES))
...
def fillDataStructureGetFunctions(outfile):
    ...
    for length in lengths:
        ...
        outfile.write('{}ArrayLockedListInt get{}(int pos, {}) {{\n'.format(SPACES,
                                                                           length,
                                                                           ", ".join(args_to_function)))
        
        # Get the Node that we need
        emitCodeDataStructureGetNodeWithReturnValue(outfile, length, 2, 'empty_list')
...
def fillDataStructureContainsSolutionFunctions(outfile):
    for variable_id in getAllSolutions():
        ...
        outfile.write('{}boolean contains_solution_{}({}) {{\n'.format(SPACES,
                                                                       variable_id.name,
                                                                       ', '.join(args)))
        
        if length == 1:
            outfile.write('{}return R_{}.contains(x_1);\n'.format(SPACES * 2, variable_id.name))
            outfile.write('{}}}\n\n'.format(SPACES))
            continue
        
        # Get the Node that we need
        emitCodeDataStructureGetNodeWithReturnValue(outfile, length, 2, 'false')
        
        outfile.write('\n')
        
        outfile.write('{}return (node.solution_{}.get(x_{}));\n'.format(SPACES * 2,
                                                                  variable_id.name,
                                                                  length))
        outfile.write('{}}}\n\n'.format(SPACES))
...
def fillDataStructureAppendSolutionFunctions(outfile):
    for variable_id in getAllSolutions():
        ...
        if length == 1:
            outfile.write('{}R_{}_lock.lock();\n'.format(SPACES * 2, variable_id.name))
            outfile.write('{}R_{}.add(x_1);\n'.format(SPACES * 2, variable_id.name))
            outfile.write('{}R_{}_lock.unlock();\n'.format(SPACES * 2, variable_id.name))
            outfile.write('{}}}\n\n'.format(SPACES))
            continue
...
def fillDataStructureCheckAndUpdateFunctions(outfile):
    for variable_id in getAllSolutions():
        ...
        args = ('int x_{}'.format(str(x)) for x in xrange(1, length+1))
        outfile.write('{}boolean check_and_update_solution_{}({}) {{\n'.format(SPACES,
                                                                               variable_id.name,
                                                                               ', '.join(args)))
        outfile.write('{}boolean r = false;\n\n'.format(SPACES * 2))
        
        if length == 1:
            outfile.write('{}R_{}_lock.lock();\n'.format(SPACES * 2, variable_id.name))
            outfile.write('{}if (!R_{}.contains(x_1)) {{\n'.format(SPACES * 2, variable_id.name))
            outfile.write('{}R_{}.add(x_1);\n'.format(SPACES * 3, variable_id.name))
        ...
        outfile.write('{}node.solution_{}.lock();\n'.format(SPACES * 2,
                                                            variable_id.name))
        outfile.write('{}if (!node.solution_{}.get(x_{})) {{\n'.format(SPACES * 2,
                                                                       variable_id.name,
                                                                       length))
        outfile.write('{}node.solution_{}.set(x_{});\n'.format(SPACES * 3,
                                                               variable_id.name,
...
def fillSolverLocalVariables(outfile):
    outputTuples = GenerationData.answersToStore
    
    outfile.write("{}private int num_threads;\n".format(SPACES))
    outfile.write("{}private Datastructure data;\n".format(SPACES))
    
    outfile.write("{}private Writer".format(SPACES))
...
def fillWriteRewritingVariable(outfile):
    allPredicates = getAllPredicatesIDs()
    
    outfile.write('{}private void write_rewriting_variable(Writer fp, RewritingVariable r) {{\n'.format(SPACES))
    outfile.write('{}String s = new String();\n\n'.format(SPACES * 2))
    ...
    for pos, predicate in enumerate(allPredicates):
        ...
        values_args = ' + ", " + '.join('r.values[{}]'.format(x) for x in xrange(getPredicateLength(predicate)))
        outfile.write('{}{}{} {{\n'.format(SPACES * 2, if_str, if_cond))
        outfile.write('{}s = "{}(" + {} + ").\\n";\n'.format(SPACES * 3,
                                                          predicate.name,
                                                          values_args))
        outfile.write('{}}}\n'.format(SPACES * 2))
    ...
    outfile.write('{}try {{\n'.format(SPACES * 2))
    outfile.write('{}fp.write(s);\n'.format(SPACES * 3))
    outfile.write('{}}} catch (IOException e) {{\n'.format(SPACES * 2))
    outfile.write('{}e.printStackTrace();\n'.format(SPACES * 3))
    outfile.write('{}}}\n'.format(SPACES * 2))
    outfile.write('{}}}\n'.format(SPACES))
...
def fillSolverInitFunction(outfile):
    ...
    outfile.write('{}boolean init(int num_threads) {{\n'.format(SPACES))
    outfile.write('{}this.num_threads = num_threads;\n'.format(SPACES * 2))
    outfile.write('{}data = new Datastructure();\n\n'.format(SPACES * 2))
    
    outfile.write('{}try {{\n'.format(SPACES * 2))
    for (pred_name, _) in outputTuples:
        outfile.write('{}fp_{} = new BufferedWriter(new OutputStreamWriter(\n'.format(SPACES * 3, pred_name))
        outfile.write('{} new FileOutputStream("{}.tuples"), "utf-8"));\n'.format(SPACES * 6, pred_name))
...
def fillSolverComputeFunction(outfile):
    ...
    outfile.write('{}boolean compute(){{\n'.format(SPACES))
    
    if number_of_stratums > 1:
        outfile.write('{}int queue_id;\n'.format(SPACES * 2))
        
    outfile.write('{}@SuppressWarnings("unchecked")\n'.format(SPACES * 2))
    outfile.write('{}Queue<RewritingVariable> [] queues = new Queue[num_threads];\n'.format(SPACES * 2))
    for x in xrange(2, number_of_stratums + 1):
        # Java line of code split on two python sentnces due to its length
        outfile.write('{}Queue<RewritingVariable> queue_stratum_{} = '.format(SPACES * 2, x))
        outfile.write('new ConcurrentLinkedQueue<RewritingVariable>();\n')
    outfile.write('{}Thread threads[] = new Thread[num_threads];\n\n'.format(SPACES * 2))
        
    
    outfile.write('{}for (int i=0; i<num_threads; i++) {{\n'.format(SPACES * 2))
    outfile.write('{}queues[i] = new LinkedList<RewritingVariable>();\n'.format(SPACES * 3))
    outfile.write('{}}}\n\n'.format(SPACES * 2))
    ...
    for stratum in xrange(1, number_of_stratums + 1):
        ...
        if stratum > 1:
            ...
            outfile.write('{}while (!queue_stratum_{}.isEmpty()) {{\n'.format(SPACES * 2, stratum))
            outfile.write('{}queues[queue_id].add(queue_stratum_{}.poll());\n'.format(SPACES * 3, stratum))
        ...
        if other_stratum_queues:
            outfile.write('{}threads[i] = new Thread(new compute_stratum_{}(i, queues[i], data, {}));\n'.format(SPACES * 3,
                                                                                                                stratum,
                                                                                                                other_stratum_queues))
        else:
            outfile.write('{}threads[i] = new Thread(new compute_stratum_{}(i, queues[i], data));\n'.format(SPACES * 3,
                                                                                                            stratum))
        outfile.write('{}threads[i].start();\n'.format(SPACES * 3));
        outfile.write('{}}}\n\n'.format(SPACES * 2))

        outfile.write('{}for (int i = 0; i < num_threads; i++) {{\n'.format(SPACES * 2))
        outfile.write('{}try {{\n'.format(SPACES * 3))
        outfile.write('{}threads[i].join();\n'.format(SPACES * 4));
        outfile.write('{}}} catch (InterruptedException e) {{\n'.format(SPACES * 3))
        outfile.write('{}e.printStackTrace();\n'.format(SPACES * 4))
        outfile.write('{}}}\n'.format(SPACES * 3))
        outfile.write('{}}}\n\n'.format(SPACES * 2))
...
def fillSolverInitStratumFunctions(outfile):
    ...
    for stratum_level in xrange(1, number_of_stratums + 1):
        ...
        if not negated_predicates:
            outfile.write('{}private void init_stratum_{}(Queue<RewritingVariable> [] queues) {{\n'.format(SPACES,
                                                                                                           stratum_level))
        else:
            outfile.write('{}private void init_stratum_{}(Queue<RewritingVariable> [] queues, Datastructure data) {{\n'.format(SPACES,
                                                                                                                               stratum_level))
            
        outfile.write('{}File file = null;\n'.format(SPACES * 2))
        outfile.write('{}BufferedReader reader = null;\n'.format(SPACES * 2))
        outfile.write('{}int queue_id;\n\n'.format(SPACES * 2))
        ...
        for idVar in idVars:
            ...
            outfile.write('{}/* {}.tuples file */\n'.format(SPACES * 2, idVar.name))
            outfile.write('{}try {{\n'.format(SPACES * 2))
            outfile.write('{}file = new File("{}.tuples");\n'.format(SPACES * 3, idVar.name))
            outfile.write('{}reader = new BufferedReader(new FileReader(file));\n'.format(SPACES * 3))
            outfile.write('{}String text = null;\n'.format(SPACES * 3))
            outfile.write('{}queue_id = 0;\n\n'.format(SPACES * 3))
            
            outfile.write('{}while ((text = reader.readLine()) != null) {{\n'.format(SPACES * 3))
            # The next line of java code has been split into two different output lines due to its length
            outfile.write('{}String [] numbers = text.substring(text.indexOf(\'(\')+1,'.format(SPACES * 4))
            outfile.write('text.indexOf(\')\')).split(",");\n')
            for var_num in xrange(1, predicate_length + 1):
                trim = '.trim()'
                if var_num == 1:
                    trim = ''
                
                outfile.write('{}int var{} = Integer.parseInt(numbers[{}]{});\n'.format(SPACES * 4,
                                                                                        var_num,
                                                                                        var_num - 1,
                                                                                        trim))
            ...
            if not (idVar in getNegatedPredicates()):
                # The next line of java code has been split into two different output lines due to its length
                outfile.write('{}queues[queue_id].add(new '.format(SPACES * 4, idVar.name))
            ...
            outfile.write('{}}}\n'.format(SPACES * 3))
            outfile.write('{}}} catch (FileNotFoundException e) {{\n'.format(SPACES * 2))
            outfile.write('{}e.printStackTrace();\n'.format(SPACES * 3))
            outfile.write('{}}} catch (IOException e) {{\n'.format(SPACES * 2))
            outfile.write('{}e.printStackTrace();\n'.format(SPACES * 3))
            outfile.write('{}}} finally {{\n'.format(SPACES * 2))
            outfile.write('{}try {{\n'.format(SPACES * 3))
            outfile.write('{}if (reader != null) {{\n'.format(SPACES * 4))
            outfile.write('{}reader.close();\n'.format(SPACES * 5))
            outfile.write('{}}}\n'.format(SPACES * 4))
            outfile.write('{}}} catch (IOException e) {{\n'.format(SPACES * 3))
            outfile.write('{}e.printStackTrace();\n'.format(SPACES * 4))
            outfile.write('{}}}\n'.format(SPACES * 3))
...
def fillSolverComputeStratumClasses(outfile):
    ...
    def common_block(spaces, equation, level, num_of_stratums, args, idToStratumLevels):
        ...
        if equation.rightVariable.id in answersToStore:
            ...
            for queue_level in idToStratumLevels[variable_id]:
                #outfile.write('{}solver_queue.add(var);\n'.format(spaces))
                if queue_level == level:
                    outfile.write('{}solver_queue.add(new RewritingVariable(VariableType.{}, {}));\n'.format(spaces,
                                                                                                             args[0],
                                                                                                             ', '.join(args[1:])))
                else:
                    outfile.write('{}queue_stratum_{}.add(new RewritingVariable(VariableType.{}, {}));\n'.format(spaces,
                                                                                                                 queue_level,
                                                                                                                 args[0],
                                                                                                                 ', '.join(args[1:])))
    ...
    for level, stratum in enumerate(GenerationData.stratums, start=1):
        ...
        outfile.write('{}private class compute_stratum_{} implements Runnable {{\n'.format(spaces_level_1,
                                                                                           level))
        outfile.write('{}private int id;\n'.format(spaces_level_2))
        outfile.write('{}private Queue<RewritingVariable> solver_queue;\n'.format(spaces_level_2))
        if (level + 1) <= number_of_stratums:
            queue_args = ', '.join('queue_stratum_{}'.format(x) for x in xrange(level + 1, number_of_stratums + 1))
            outfile.write('{}private Queue<RewritingVariable> {};\n'.format(spaces_level_2,
                                                                            queue_args))
        outfile.write('{}private Datastructure data;\n\n'.format(spaces_level_2))
        
        
        # This line of java code has been splitted into different output lines because it is too long
        outfile.write('{}public compute_stratum_{} (int id, '.format(spaces_level_2, level))
        if (level + 1) <= number_of_stratums:
            queue_args = ', '.join('Queue<RewritingVariable> queue_stratum_{}'.format(x) for x in xrange(level + 1, number_of_stratums + 1))
            outfile.write('Queue<RewritingVariable> solver_queue, Datastructure data, {}) {{\n'.format(queue_args))
        else:
            outfile.write('Queue<RewritingVariable> solver_queue, Datastructure data) {\n')
        outfile.write('{}this.id = id;\n'.format(spaces_level_3))
        outfile.write('{}this.solver_queue = solver_queue;\n'.format(spaces_level_3))
        outfile.write('{}this.data = data;\n'.format(spaces_level_3))
        for x in xrange(level + 1, number_of_stratums + 1):
            outfile.write('{0}this.queue_stratum_{1} = queue_stratum_{1};\n'.format(spaces_level_3, x))
        
        outfile.write('{}}}\n\n'.format(spaces_level_2))

        outfile.write('{}@Override\n'.format(spaces_level_2))
        outfile.write('{}public void run() {{\n'.format(spaces_level_2))
        #outfile.write('{}RewritingVariable current, var;\n\n'.format(spaces_level_3))
        fillIntList(outfile, spaces_level_3)
        outfile.write('{}RewritingVariable current;\n\n'.format(spaces_level_3))
        
        outfile.write('{}while (!solver_queue.isEmpty()) {{\n'.format(spaces_level_3))
        ...
        for variable_id in chain(block1, block2, block3):
            ...
            outfile.write('\n')
            outfile.write('{}/* Rewriting variable {} block */\n'.format(spaces_level_4, variable_id.name))
            outfile.write('{}if (current.v_type == VariableType.{})'.format(spaces_level_4,
                                                                            variable_id.name.upper()))
            outfile.write(' {\n')

            #if variable_id in outputTuples:
            #    outfile.write('{}write_rewriting_variable(fp_{}, current);\n\n'.format(spaces_level_5, variable_id.name))
...
#             outfile.write('#endif\n')
                    ...
                    if commonVars_len == 0:
                        ...
                        if (len(set([x for x in equation.consultingArguments if not (isinstance(x, Argument) and x.type=='constant')])) != 1 and\
                            ...
                            if set(filter(lambda x: isinstance(x, Argument), 
                                ...
                                outfile.write('{}if (!l1.get_array().isEmpty()) {{\n'.format(spaces))
                        else:
                            all_variables_same_equal_card = True
        ...
        # End the class
        outfile.write('{}}}\n\n'.format(spaces_level_2))
        outfile.write('{}public void finalize() {{\n'.format(spaces_level_2))
        outfile.write('{}}}\n\n'.format(spaces_level_2))
...
fill_template = {
    ...
    'fillDataStructureCreateNodeClasses' : fillDataStructureCreateNodeClasses,
    'fillDataStructureConstructor': fillDataStructureConstructor,
    ...
    'fillSolverLocalVariables' : fillSolverLocalVariables,
    'fillWriteRewritingVariable' : fillWriteRewritingVariable,
...
def generate_code_from_template(output_directory, stratums, 
    ...
    path = os.path.normpath(output_directory + '/Solver_Java_code')
